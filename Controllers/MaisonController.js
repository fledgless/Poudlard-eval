...
class MaisonController {
    async getAllMaisons(req, res) {
        try {
            const maison = await MaisonService.getAllMaisons();
            res.json(maison);
        } catch (error) {
            console.log(error);
            res.status(500);
            res.json({ error: 'Erreur lors de la récupération des maisons.' });
        }
    }
    ...
    async addMaison(req, res) {
        try {
            ...
            console.log(error);
            res.status(500);
            res.json({ error: 'Erreur lors de l\'ajout de la maison.' })
        }
    }
    ...
    async deleteMaison(req, res) {
        try {
            ...
            console.log(error);
            res.status(500);
            res.json({ error: 'Erreur lors de la suppression de la maison.' })
        }
    }
}
